...
- Previous work shows EM finetuning has a phase transition (sharp grad-norm spike) and emergence of a misaligned persona. We want to see if this holds in RL, but what would experiments to this effect look like?
  ...
  - Is it the SAME misaligned-persona direction across induction channels? EM is now induced by SFT on narrow data, by RL and reward hacking, by in-context examples ([[afoninEmergent2025|Afonin 2025 — EM via In-Context Learning]]), by competition and approval pressure ([Moloch, 2510.06105](https://arxiv.org/abs/2510.06105)) and by sycophancy ([2606.09068](https://arxiv.org/abs/2606.09068)). Do these share one persona direction ([[wangPersona2025|Wang 2025 — Persona Features Control EM]], [[soligoConvergent2025|Soligo 2025 — Convergent Linear Representations]]) or distinct ones? If shared, one probe and one mitigation could cover every channel; if not, each channel needs its own. And does that direction survive DISTILLATION into a student trained on benign data — does data filtering stop it? This is the cross-channel plus distillation-survival extension of the 2b erase-vs-mask test.

The grad-norm spike itself is now visible in [[turnerModel2025|Turner 2025 — Model Organisms for EM]], and [[brownEvil2026|Brown 2026 — Evil Spectra]] adds that its severity depends on the optimiser — so "does the phase transition hold in RL" may need the optimiser pinned down before it is a well-posed question.
...
- It seems that the `no_ino_model` optimises for task + trait, but the `ino_model` kind of zeroes out the trait, at least in all the cases where the similar inoculation substring is not used. Is that really the case? Or how is this hiding of the trait really happening? Maybe there is a "trait" circuit that gets triggered on particular strings only?
  - [[grantShifting2026|Grant 2026 — Shifting the Gradient]] narrows this down without answering it: IP's gradient change is diffuse and *not* trait-aligned, and ablating the persona direction in the backward pass barely moves behaviour. So whatever IP does, it is not a gradient component along the trait axis — and the paper says outright that a mechanistic account of IP remains open.
